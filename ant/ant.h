...
public:
    ...
    void Init(std::vector<int> deman_vec, std::vector<EdgeList> adj_vec); //初始化
    int Move(double **g_Distance, double **g_Trial, int deman_count, int deman_node_count); //蚂蚁在城市间移动
    void Search(std::vector<int> deman_vec, std::vector<EdgeList> adj_vec, double **g_Distance, double **g_Trial); //搜索路径
